  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div 
        ...
        className="relative flex aspect-[21/9] w-full cursor-crosshair flex-col items-center justify-center rounded-[2.5rem] border-2 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 hover:border-blue-500 transition-all overflow-hidden"
      >
        <div className="flex flex-col items-center gap-6 text-center pointer-events-none select-none">
          {currentRate > 0 ? (
            <>
              <div className="text-7xl font-black text-blue-600 tabular-nums">{currentRate} <span className="text-2xl font-bold text-slate-400">Hz</span></div>
              <p className="text-sm font-bold text-slate-500">마우스를 계속 움직여주세요</p>
            </>
          ) : (
            <>
              ...
              <div>
                <p className="text-xl font-bold text-slate-900 dark:text-white">마우스를 영역 안에서 움직이세요</p>
                <p className="text-sm text-slate-500">전송 속도(Polling Rate)를 실시간으로 측정합니다.</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="absolute bottom-0 left-0 flex h-24 w-full items-end justify-center gap-1 px-10 opacity-30">
          {samples.map((s, i) => (
            <div 
              key={i} 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-4">
          <div className="rounded-3xl border border-slate-100 dark:border-slate-800 p-8 bg-white dark:bg-slate-950 shadow-sm">
            <h3 className="mb-6 text-[10px] font-bold uppercase tracking-widest text-slate-500">측정 결과</h3>
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600 dark:text-slate-400">최대 속도</span>
                <span className="text-2xl font-black text-slate-900 dark:text-white tabular-nums">{maxRate} Hz</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600 dark:text-slate-400">상태</span>
                <span className={cn(
                  "text-xs font-bold px-3 py-1 rounded-full",
                  maxRate >= 1000 ? "bg-emerald-500/10 text-emerald-500" : "bg-slate-500/10 text-slate-500"
                )}>
                  {maxRate >= 1000 ? "고성능" : "일반"}
                </span>
              </div>
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="rounded-2xl bg-amber-50/50 dark:bg-amber-900/10 p-6 border border-amber-100 dark:border-amber-900/20">
            <h4 className="mb-2 flex items-center gap-2 text-xs font-bold text-amber-700 dark:text-amber-400">
          ...
          <div className="rounded-2xl border border-slate-100 dark:border-slate-800 p-6 flex items-start gap-4 bg-slate-50/50 dark:bg-slate-900/20">
            ...
            <div>
              <h4 className="mb-1 text-sm font-bold text-slate-900 dark:text-white">왜 중요할까요?</h4>
